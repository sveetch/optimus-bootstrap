...
class PostGenerationHookManager:
    ...
    def repository_init(self, adds=["*"]):
        """
        Initialize repository and possibly make initial commit with project files.

        Arguments:
            adds (list): List of strings for paths to add and staged to commit. Default
                to ``*``. If empty value is given, no path will be staged to commit and
                so there won't be an initial commit.
        """
        print()
        ...
        if adds:
            ...
            self.execute_commandline("git", "add", *adds)
...
if __name__ == "__main__":
    ...
    if context["init_git_repository"]:
        manager.repository_init([".gitignore", "*"])
